...
const ShoppingBasketModal = forwardRef((props, ref) => {
	...
	// ANY
	const itemList = useSelector((state) => (state as any).character.itemList);

	const shopList = useSelector((state) => (state as any).character.shopList);
	...
	const currentPoint = useSelector(
		(state) => (state as any).character.currentPoint
	);
	...
	// 미리보기 리스트
	const previewBg = useSelector(
		(state) => (state as any).character?.backgroundItem
	);
	const previewColor = useSelector(
		(state) => (state as any).character?.colorItem
	);
	const previewClothes = useSelector(
		(state) => (state as any).character?.clothesItem
	);
	const previewAcc = useSelector((state) => (state as any).character?.accItem);
	const previewEmotion = useSelector(
		(state) => (state as any).character?.emotionItem
	);
	...
	// ANY
	const [shopBg, setBg] = useState<ItemProps>();
	const [shopColor, setColor] = useState<ItemProps>();
	const [shopClothes, setClothes] = useState<ItemProps>();
	const [shopAcc, setAcc] = useState<ItemProps>();
	const [shopEmotion, setEmotion] = useState<ItemProps>();
