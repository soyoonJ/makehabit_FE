...
const RankingPage = () => {
	...
	const MyNickname = useSelector(
		(state) => (state as any).ranking.my_list?.nickname
	);
	const MyRank = useSelector((state) => (state as any).ranking.my_list?.rank);
	const MyCnt = useSelector(
		(state) => (state as any).ranking.my_list?.proofCnt
	);

	//아이템 가져오기
	const isEquip = useSelector(
		(state) => (state as any).ranking.my_list?.equippedItems
	);
